...
<tr>
<td align="center"><span style="font-family: 'Noto Sans'; font-style: normal; font-weight: 500; font-size: 24px;line-height: 140%;text-align: center; color: #FFFFFF;">Регистрация<br><br></td>
</tr>
...
<br>
<input name="uname" placeholder="Логин" type="text" />
</td>
</tr>
<tr style="display:none">
<td><span style="font-family: 'Noto Sans'; font-style: normal; font-weight: 400; font-size: 10px; line-height: 140%;color: #7E8486; flex: none; order: 0; flex-grow: 0;">E-mail</span>
<br>
...
<tr>
<td><span style="font-family: 'Noto Sans'; font-style: normal; font-weight: 400; font-size: 10px; line-height: 140%;color: #7E8486; flex: none; order: 0; flex-grow: 0;">Повторить пароль</span>
<br>
<input name="pwd2" placeholder="******" type="password" />
</td>
</tr>
<tr>
<td><br><br><button id="loginbtn" >Создать аккаунт</button></td>
</tr>
</form>
<tr>
<td align="center"><br><br><span style="font-family: 'Noto Sans'; font-style: normal; font-weight: 400; font-size: 14px; line-height: 19px; align-items: center; text-align: center; color: #FFFFFF;">Уже есть аккаунта?</span></td>
</tr>
<tr>
<td><button onclick="window.location.href='index.php'" style="box-sizing: border-box; cursor:pointer; background-color:transparent; color:white;  width: 338px; height: 38px; border: 2px solid #F1956F; border-radius: 10px;">Войти</button></td>
</tr>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    die ("CSRF error");
		...
		if (isset($_POST['uname']) && isset($_POST['pwd']) && isset($_POST['pwd2']) && ($_POST['pwd']==$_POST['pwd2']))

		{
			$pdo = new PDO('sqlite:testdb.sqlite');
			$sql = "INSERT INTO sysuserrole(userrole) VALUES ('')";
			$stmt = $pdo->prepare($sql);
			$stmt->execute();
			$lastId = $pdo->lastInsertId();
			
			// 查询 sysuserrole 表以获取正确的角色ID
			$sqlGetRoleId = "SELECT id FROM sysuserrole WHERE id = :id";
			$stmtGetRoleId = $pdo->prepare($sqlGetRoleId);
			$stmtGetRoleId->bindParam(':id', $lastId);
			$stmtGetRoleId->execute();
			$roleId = $stmtGetRoleId->fetchColumn();
			
			// 插入 sysuser 表
			$sql1 = "INSERT INTO sysuser(login, password, type, role, permission) VALUES(:uname, :pwd, '2', :roleId, '1')";
			$stmt1 = $pdo->prepare($sql1);
			$stmt1->bindParam(':uname', $_POST['uname']);
			$stmt1->bindParam(':pwd', $_POST['pwd']);
			$stmt1->bindParam(':roleId', $roleId);
			$stmt1->execute();
		}
